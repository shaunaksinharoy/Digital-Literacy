# Digital-Literacy
# Digital Literacy Portfolio

## Student Digital Ambassador Project

**Course:** CSE0001 - Digital Literacy  

---

## Student Information

| Field | Details |
|-------|---------|
| **Name** | Shaunak Sinha Roy|
| **Registration Number** | 25BEC10032 |
| **Branch** | ECE |
| **Year** | BTech First Year |

---

## Project Overview

This repository serves as my Digital Literacy Portfolio, created as part of the CSE0001 course project. In the scenario, as a Student Digital Ambassador, my goal is to help peers navigate the digital landscape safely and professionally.

This portfolio showcases my work across five key digital literacy modules. It includes visual resources, professional profiles, practical exercises, and awareness materials—all organized to demonstrate a comprehensive understanding of digital tools, online safety, and professional communication.

---

## Repository Structure & Task Summary

The project is organized into five tasks, each corresponding to a course module. All deliverables are contained within the folders linked below.

| Task | Module | Deliverables |
|------|--------|--------------|
| **Task 1** | Digital Literacy Awareness | Infographic (Canva) |
| **Task 2** | Build Your Student Digital Portfolio | GitHub Profile, LinkedIn, Kaggle | 
| **Task 3** | Coding website, Google Form & Sheet link | HackerRank, Digital Awareness quiz|
| **Task 4** | Email Etiquette & Social Media | Professional email drafts, Social Media Do's/Don'ts checklist | 
| **Task 5** | Cybercrime Awareness | Case study, Prevention checklist | 

---

## Quick Links & Resources

| Task | Tool / Platform | Link / Resource |
|------|-----------------|-----------------|
| Task 1 | Canva (Infographic) |  |
| Task 2 | GitHub Profile |  |
| Task 2 | LinkedIn Profile |  |
| Task 3 | Coding Platform (HackerRank) |  |
| Task 3 | Google Form |  |
| Task 5 | National Cyber Crime Portal | |

---

## Contact

For any questions or feedback regarding this project, please feel free to reach out.

- **Email:** shaunak.25bec10032@vitbhopal.ac.in
- **GitHub:** 
- **LinkedIn:** 

---
